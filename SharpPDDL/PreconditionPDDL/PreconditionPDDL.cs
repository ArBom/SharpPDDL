...
namespace SharpPDDL
{
    ...
    internal class PreconditionPDDL : ObjectPDDL
    {
        ...
        internal static PreconditionPDDL Instance<T1c, T1p, T2c, T2p>(string Name, List<Parametr> Parameters, List<PreconditionPDDL> Preconditions, ref T1c obj1, Expression<Func<T1p, T2p>> func, ref T2c obj2)
            where T1p : class
            where T2p : class
            where T1c : class, T1p
            where T2c : class, T2p
